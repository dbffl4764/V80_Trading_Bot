...
# 2. 사령관님 코드 기반 + 20배 레버리지 + 고점대비 1% 익절 로직 주입
cat << 'EOF' > main.py
        ...
        try:
            ...
            while True:
                ...
                if not self.half_profit_taken and roe >= 100.0:
                    ...
                    self.ex.create_order(symbol, 'STOP_MARKET', 'sell' if side == "LONG" else 'buy', half_amt, None, {'stopPrice': self.ex.price_to_precision(symbol, safe_p), 'reduceOnly': True})
                    self.half_profit_taken = True
                    self.log(f"💰 [1차 익절] 100% 달성! 50% 던지고 수익 방어선 구축.")
                ...
                if self.half_profit_taken:
                    ...
                    if drop >= 1.0:
                        ...
                        self.log(f"🏁 [최종 익절] 고점 대비 1% 하락! 수익 똑바로 챙겼습니다. ROE: {roe:.2f}%")
                        break
        except Exception as e: self.log(f"⚠️ 에러: {e}")

    def run(self):
        self.log(f"⚔️ V80 [20배/수익사수] 가동.")
        while True:
            try:
