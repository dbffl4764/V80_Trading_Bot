...
class V80_Infinite_Striker:
    ...
    def check_v80_signal(self, symbol):
        """서열 파괴 절대 금지: 5일선이 밑에 있으면 롱 절대 불가"""
        try:
            ...
            # [LONG] 완벽 정배열 서열: 5일선이 20일선 위에, 20일선이 60일선 위에!
            is_perfect_long = (ma5 > ma20) and (ma20 > ma60)
            
            # [SHORT] 완벽 역배열 서열: 5일선이 20일선 아래에, 20일선이 60일선 아래에!
            is_perfect_short = (ma5 < ma20) and (ma20 < ma60)

            gap = abs(curr - ma20) / ma20 * 100

            if gap <= 2.5:
                if is_perfect_long and curr > ma20: # 5일선이 역배열이면 여기서 무조건 걸러짐
                    self.log(f"💎 [서열확인] {symbol} 정배열 롱 타점 (5 > 20 > 60)")
                    return "LONG", curr
                
                elif is_perfect_short and curr < ma20:
                    self.log(f"💀 [서열확인] {symbol} 역배열 숏 타점 (5 < 20 < 60)")
                    return "SHORT", curr
