...
class V80_Full_Strategy:
    def __init__(self):
        ...
        self.leverage = 20
        self.max_entry_count = 3  # [복구] 3분할 사격

    def log(self, msg):
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🛡️ {msg}")

    def run(self):
        self.log("🚀 3분할 사격 + 10% 변동성 필터 엔진 기동")
        while True:
            try:
                ...
                # 10% 변동성 종목만 타겟팅
                targets = [s for s, t in ts.items() if s.endswith('/USDT:USDT') and abs(t.get('percentage', 0)) >= 10.0]
                        ...
                        if side:
                            ...
                            while True:
                                ...
                                if not half_taken and roe >= 100.0:
                                    ...
                                    half_taken = True
                                    self.log(f"💰 [반익절] 100% 달성. 수익 사수 시작.")

                                # 고점대비 1% 하락시 전량 익절
                                if half_taken:
                                    drop = (high_p - now_p) / high_p * 100 if side == 'buy' else (now_p - high_p) / high_p * 100
                                    if drop >= 1.0:
                                        self.ex.create_market_order(s, 'sell' if side == 'buy' else 'buy', curr_amt, {'reduceOnly': True})
